...
def check_winner(game):
    ...
    return 0  # Fixed error: return 0 if no winner is found
...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player  # Fixed error: removed column+1
    return game
...
def switch_player(player):
    if player == 1:# Fixed error: changed = to ==
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available:# Fixed error: added colon
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row, column) #added column parameter
